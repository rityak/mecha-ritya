...
@merge_method(identifier="karcher_mean", register=True)
def karcher_mean(*tensors: Parameter(Tensor),
    ...
    try:
        alpha_values = [float(a.strip()) for a in alphas.split(',')]
    except (ValueError, TypeError) as e:
        raise ValueError(f"Invalid weight format: '{alphas}'. Weights must be numbers separated by commas.")
